...
void APlaytestRecorderActor::Tick(float DeltaSeconds)
{
    ...
    {
        // F9 conflicts with an Unreal Editor wireframe shortcut in PIE. F12 is
        // intentionally used instead; Unreal's own screenshot behavior is
        // harmless and aligns with the marker's purpose.
        if (PlayerController->WasInputKeyJustPressed(EKeys::F12))
        {
            RecordIssueMarker(TEXT("F12 manual playtest marker"));
        }
    }
}
...
void APlaytestRecorderActor::WriteSessionMetadata(const FString& Status, const FString& EndedUtc)
{
    ...
    FFileHelper::SaveStringToFile(
        ...
        *FPaths::Combine(SessionDirectory, TEXT("session.json")),
...
void APlaytestRecorderActor::RecordIssueMarker(const FString& Note)
{
    ...
    if (GEngine)
    {
        GEngine->AddOnScreenDebugMessage(
            ...
            2.5f,
            FColor::Yellow,
            FString::Printf(TEXT("Playtest marker #%d captured"), MarkerCount));
    }
}
