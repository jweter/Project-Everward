...
foreach ($CommonPath in @("C:\Program Files\Epic Games\UE_5.8", "C:\Epic Games\UE_5.8")) {
    $Candidates.Add($CommonPath)
}

$EditorCmd = $null
foreach ($Candidate in $Candidates | Select-Object -Unique) {
    $CandidateCmd = Join-Path $Candidate "Engine\Binaries\Win64\UnrealEditor-Cmd.exe"
    if (Test-Path $CandidateCmd -PathType Leaf) {
        $EditorCmd = (Resolve-Path $CandidateCmd).Path
        break
    }
}
if (-not $EditorCmd) {
    throw "Unreal Engine 5.8 command-line editor was not found."
}
