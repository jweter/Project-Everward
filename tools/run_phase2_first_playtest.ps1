...
if ((Test-Path $DedicatedSentinel -PathType Leaf) -and (Test-Path $WorkerRegistration -PathType Leaf)) {
    try {
        ...
        if ($LASTEXITCODE -ne 0) { Write-Warning "Everward unattended worker registration returned exit code $LASTEXITCODE. Manual playtest will continue." }
    }
    catch { Write-Warning "Everward unattended worker registration failed: $($_.Exception.Message). Manual playtest will continue." }
}
...
function Resolve-Unreal58Root {
    ...
    foreach ($RegistryPath in @("HKLM:\SOFTWARE\EpicGames\Unreal Engine\5.8", "HKCU:\SOFTWARE\EpicGames\Unreal Engine\5.8", "HKLM:\SOFTWARE\WOW6432Node\EpicGames\Unreal Engine\5.8")) {
        try {
    ...
    foreach ($Candidate in $Candidates | Select-Object -Unique) {
        ...
        if ((Test-Path $BuildBat) -and (Test-Path $EditorExe)) { return (Resolve-Path $Candidate).Path }
    }
...
function Set-ObservationCheck {
    param([string]$ObservationPath, [string]$CheckId, [ValidateSet("pass", "fail", "not_tested")][string]$Status, [string]$Notes)
    ...
    if ($Status -eq "fail") { $Data.overall_result = "fail" }
    elseif ($Data.overall_result -eq "not_tested") { $Data.overall_result = "partial" }
...
function Set-LowSpecMemoryEvidence {
    param([string]$ObservationPath, [double]$PeakWorkingSetMiB, [int]$SampleWindowSeconds)
    $Data = Get-Content -Raw -Path $ObservationPath | ConvertFrom-Json
    $Evidence = [pscustomobject]@{
        metric = "editor_peak_working_set_mib"
        value = $PeakWorkingSetMiB
    ...
    $Data | Add-Member -NotePropertyName low_spec_memory -NotePropertyValue $Evidence -Force
    $Data | ConvertTo-Json -Depth 12 | Set-Content -Path $ObservationPath -Encoding UTF8
}
...
try { $Gpu = (Get-CimInstance Win32_VideoController | Select-Object -First 1 -ExpandProperty Name).Trim() } catch {}
...
if ($NeedsManualLock) { $PID.ToString() | Set-Content -Path $ManualPlaytestLock -Encoding ASCII }

try {
    & $Python $TemplateHelper --output $ObservationPath --git-commit $GitCommit --cpu $Cpu --gpu $Gpu
    if ($LASTEXITCODE -ne 0) { throw "Failed to prepare the Phase 2 first-run observation file." }
    ...
    else { Write-Host "Build skipped by request; unreal_cpp_build remains not_tested." }
    ...
    if (-not $NoLaunch) {
        ...
        if ($LowSpec -and $MeasureMemory) {
            ...
            while ((Get-Date) -lt $MemoryDeadline -and -not $EditorProcess.HasExited) {
                $EditorProcess.Refresh()
                $PeakWorkingSetBytes = [Math]::Max($PeakWorkingSetBytes, $EditorProcess.WorkingSet64)
                Start-Sleep -Milliseconds 500
            }
            $PeakWorkingSetMiB = [Math]::Round($PeakWorkingSetBytes / 1MB, 1)
            Set-LowSpecMemoryEvidence -ObservationPath $ObservationPath -PeakWorkingSetMiB $PeakWorkingSetMiB -SampleWindowSeconds $SampleWindowSeconds
            Write-Host ("Measured UnrealEditor peak working set during the first {0}s: {1} MiB" -f $SampleWindowSeconds, $PeakWorkingSetMiB)
        }
        Write-Host "Unreal Editor launched. Follow docs/PHASE2_FIRST_RUN_PLAYTEST.md and update the observation file as you test."
    }
    else { Write-Host "Editor launch skipped by request." }
    Write-Host ""; Write-Host "Validate the completed observation with:"; Write-Host "  python tools\validate_phase2_first_run_observation.py `"$ObservationPath`""
}
finally {
    if ($NeedsManualLock -and (Test-Path $ManualPlaytestLock -PathType Leaf)) { Remove-Item -Force $ManualPlaytestLock -ErrorAction SilentlyContinue }
}
