...
- `temperature_k` reaching a new `overheat_temperature_k` threshold sets `is_overheating` and degrades `can_scan`/`can_thrust` to `false`, rejecting new `start_scan`/`set_velocity_mps` commands until it cools back below the threshold, emitting `OverheatBegan`/`OverheatEnded` domain events on the respective transitions;
...
- complete component model for sensors, computation, propulsion, and thermal behavior beyond the power-allocation budget, energy-consumption effect, thermal-load/passive-cooling effect, and overheat-response effect above (e.g. component operational state, failure modes, a behavioral response to full energy depletion, forcibly interrupting an in-progress scan/maneuver on overheat);
