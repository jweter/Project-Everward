...
Remaining candidates for this same golden/pinned-output pattern in a future small slice: `prototypes/simulation-clock` and `prototypes/coordinate-scale` currently rely on round-trip/property-style assertions rather than pinned canonical output, which is a reasonable fit for their purely arithmetic contracts but is worth re-checking if either module grows a non-trivial derived/hashed value.
