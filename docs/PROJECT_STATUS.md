...
5. begin power allocation and component-state mechanics — **in progress in `src/simulation/`**: `SimulationCore::allocate_power` validates and sets a per-subsystem (sensors/propulsion/computation/thermal) share of a total `power_capacity_w` budget and emits `PowerAllocationChanged`; allocated power now also draws down `stored_energy_j` over simulated time on the fixed-step path, clamped at zero, emitting `EnergyDepleted` on the depletion transition; allocated power now also accumulates as waste heat into `temperature_k` over the same fixed-step path via a new `thermal_capacity_j_per_k` field; `temperature_k` now also cools passively back toward a new `ambient_temperature_k` field at a rate set by a new `passive_cooling_w_per_k` field, so sustained allocated power drives `temperature_k` toward a finite equilibrium instead of climbing unbounded, and zero allocated power lets it drift back down to ambient; `temperature_k` crossing a new `max_operating_temperature_k` field now also has a behavioral consequence — it sets a new `is_overheated` flag and emits `OverheatStarted`, restoring it and emitting `OverheatEnded` once it drops back below the limit; `stored_energy_j` reaching zero now similarly sets a new `is_energy_depleted` flag (reusing the existing `EnergyDepleted` event as its transition marker, with no recovery branch since no recharge mechanic exists yet); `can_scan`/`can_thrust` are now derived once per fixed step from **both** flags combined (`is_overheated || is_energy_depleted`) via `refresh_capability_lockouts()`, so the two lockout causes correctly stack instead of either response's own recovery wrongly restoring capabilities while the other cause is still active. Still pending within this item: any energy-recharge/generation mechanic (which would give `is_energy_depleted` a recovery path), broader per-component operational/failure state beyond these two probe-wide lockouts (e.g. what happens once compute or propulsion themselves fail rather than the probe as a whole), and Blueprint/adapter exposure of all three commands once item 1 exists;
6. continue until the Phase 2 gate is demonstrably true: **simply existing as the probe is compelling.**

The next highest-value engine-independent slice within item 5 is its own new, currently-unspecified mechanical rule and should be scoped as its own deliberate slice: either (a) an energy-recharge/generation mechanic (e.g. solar or RTG-style passive charging) that would give `is_energy_depleted` a real recovery path to design and test, or (b) starting the broader per-component operational/failure state named above (e.g. what happens once an individual subsystem — compute, propulsion, sensors — fails independently of the two current probe-wide lockouts). Absent a clearer roadmap signal for which of those two is higher-value, prefer (a): it is the more self-contained closed-form addition (mirrors the existing draw-down/depletion pattern in reverse) and directly completes the asymmetry `is_energy_depleted` was deliberately left with in this slice, whereas (b) is a larger, less-specified design surface better scoped with more context.
