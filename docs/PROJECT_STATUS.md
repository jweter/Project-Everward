...
- `ProbeSimulationAdapter.cpp` wires the real guard (`make_hull_self_collision_guard()`) into its `ManipulatorRig`;
- unit coverage (`everward_manipulator_hull_contact_tests`): the idle fully-deployed rest pose does not false-positive against the hull; folding the shoulder to its -90 degree limit does intersect the hull; a rig built with the real guard cannot reach that folded pose (stops short, and the pose it stops at is verified non-intersecting); deploy/stow still completes normally with the guard installed; an ordinary 45 degree shoulder swing with a full elbow bend still reaches its exact commanded target (no false-positive freeze). All 11 `src/simulation` ctest suites pass.

This does not cover arm/environment collision (an arm sweeping into an external registered body) — only arm/probe-body self-collision, which is what the player-visible result names. **Status: implemented, Product Reality pending** for the self-collision behavior itself (confirming a commanded fold-in visibly stops rather than clips, in the actual UE build); arm/environment collision remains a stated but unimplemented requirement.
