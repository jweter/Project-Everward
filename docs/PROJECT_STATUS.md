...
A follow-up slice (branch `claude/upbeat-lamport-t12rr5`) closes the temperature-limit/overheat gap named above: `temperature_k` now has a defined overheat threshold and a discrete, hysteresis-protected capability response instead of only ever accumulating without consequence.

- two new `ProbeStateSnapshot` fields, `overheat_threshold_k` (default `358.15`, 85 degC) and `overheat_recovery_margin_k` (default `5.0` K), plus a new `is_overheated` state flag (default `false`): the threshold is set well above the default `ambient_temperature_k` (`293.15`) so ordinary brief/low-power operation never approaches it, but stays comfortably reachable within the probe's real power budget — given the default `passive_cooling_w_per_k` (`2.0`) and `thermal_capacity_j_per_k`, any sustained total allocated power above 130 W eventually equilibrates at or above the threshold, well within the default 750 W `power_capacity_w`;
- `SimulationCore::advance_wall_ticks` now also calls a new `integrate_overheat_response`, immediately after `integrate_thermal_load` on the same fixed-step path: crossing the threshold (ambient → overheat transition) sets `can_scan` and `can_thrust` false and emits a new `Overheated` domain event exactly once on that transition, mirroring `EnergyDepleted`'s transition-only pattern rather than firing on every step spent overheated;
- capability gating reuses the exact existing mechanism rather than adding a new one: `start_scan` and `set_velocity_mps` already reject calls when `can_scan`/`can_thrust` are false, so the overheat response needed no new validation logic at either call site — an in-progress scan or existing velocity is not forcibly cancelled, only new commands are blocked;
- recovery (the reverse transition) does not fire at the same threshold: temperature must fall to `overheat_threshold_k - overheat_recovery_margin_k` (5 K of hysteresis) before `can_scan`/`can_thrust` are restored and a new `OverheatRecovered` event fires, exactly once, to avoid rapid on/off toggling of capability and duplicate events from small fluctuations right at the boundary;
- new CMake/CTest coverage exercises the no-op case (no allocated power never approaches the threshold), the happy-path crossing under sustained power with `Overheated` firing exactly once and not re-firing on subsequent steps spent overheated, the recovery transition restoring capability and firing `OverheatRecovered` exactly once, and a boundary/exact-threshold case that binary-searches the exact whole-tick crossing point and confirms the transition is exactly discrete (not overheated and no event one tick short of it; overheated with the event fired exactly once at it).

This intentionally does not implement any behavioral response to full energy depletion (e.g. degrading `can_thrust`/`can_scan` once `stored_energy_j` reaches zero) — that remains its own, independently-scoped follow-up, as previously noted — and does not yet touch `unreal/`: `UProbeSimulationAdapter` does not expose `is_overheated`/`Overheated`/`OverheatRecovered` to Blueprint, and there is no embodied probe runtime scene yet for any of this to be presented through.
...
5. begin power allocation and component-state mechanics — **in progress in `src/simulation/`**: `SimulationCore::allocate_power` validates and sets a per-subsystem (sensors/propulsion/computation/thermal) share of a total `power_capacity_w` budget and emits `PowerAllocationChanged`; allocated power now also draws down `stored_energy_j` over simulated time on the fixed-step path, clamped at zero, emitting `EnergyDepleted` on the depletion transition; allocated power now also accumulates as waste heat into `temperature_k` over the same fixed-step path via a new `thermal_capacity_j_per_k` field; `temperature_k` now also cools passively back toward a new `ambient_temperature_k` field at a rate set by a new `passive_cooling_w_per_k` field, so sustained allocated power drives `temperature_k` toward a finite equilibrium instead of climbing unbounded, and zero allocated power lets it drift back down to ambient; `temperature_k` now also has a defined `overheat_threshold_k` and, once crossed, degrades `can_scan`/`can_thrust` and fires `Overheated`, recovering (with `overheat_recovery_margin_k` hysteresis) and firing `OverheatRecovered` once temperature falls back below the recovery point — **done**, see the dated paragraph above. Still pending within this item: a behavioral response to full energy depletion (still its own, independently-scoped follow-up — not folded into the overheat work), broader per-component operational/failure state beyond thermal/energy, and Blueprint/adapter exposure of all commands once item 1 exists;
6. continue until the Phase 2 gate is demonstrably true: **simply existing as the probe is compelling.**

The next highest-value engine-independent slice within item 5 is a behavioral response to full energy depletion (e.g. degrading `can_thrust`/`can_scan` once `stored_energy_j` reaches zero) — this is its own new, currently-unspecified mechanical rule (energy depletion and overheating are different failure conditions with potentially different appropriate responses) and should be scoped and reasoned about deliberately rather than assumed identical to the overheat response now in place. Beyond that, item 5 is otherwise now substantially complete at the `src/simulation/` layer pending Blueprint/adapter exposure, which depends on item 1's Unreal-side bootstrap.
