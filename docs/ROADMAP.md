...
**Implementation foundation:** `src/simulation/include/everward/simulation/fix_it.hpp` now provides the first simulation-authoritative `Fix_It` kernel over the existing component-integrity model. It encodes the canonical internal program identity/directive, preservation-first repair ordering, minimum-useful restoration bands, explicit material/energy/time estimates, transactional staged repair, and physical Generation-1 component replacement recipes/execution using authoritative stored material and energy. Replacement fabrication leaves the old component mechanically authoritative until fabrication/installation completes. Upgrade -> Redesign -> Evolution are explicit downstream design/execution gates; successor construction, lineage, and consciousness-transfer mechanics remain later systems and must not be inferred from this foundation alone.
