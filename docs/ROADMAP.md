...
**Implementation foundation:** `src/simulation/include/everward/simulation/fix_it.hpp` now provides the first simulation-authoritative `Fix_It` kernel over the existing component-integrity model. It encodes the canonical internal program identity/directive, preservation-first repair ordering, minimum-useful restoration bands, explicit material/energy/time estimates, and gated handoffs through Replacement -> Upgrade -> Redesign -> Evolution. This is the deterministic planning foundation only; actual fabrication/installation, redesign execution, successor construction, lineage, and consciousness-transfer mechanics remain later explicit systems and must not be inferred from the planner alone.
