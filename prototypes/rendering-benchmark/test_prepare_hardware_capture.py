...
class HardwareCapturePreparationTests(unittest.TestCase):
    def test_real_repository_layout_prepares_unreal_capture_workspace(self):
    ...
    def test_existing_unreal_evidence_is_not_overwritten_by_default(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            output_dir = Path(temp_dir) / "capture"
            output_dir.mkdir()
            existing = output_dir / "unreal-run-record.json"
